...
function interpretCaptchaResponse(http:Response | error response) returns @tainted boolean | error {
    if (response is http:Response) {
        map<json> msg = <map<json>> response.getJsonPayload();
        log:printDebug(io:sprintf("Recaptcha validation response : %d", check string.constructFrom(msg)));
        boolean success = check boolean.constructFrom(msg["success"]);
        if (success) {
...
public function isEngaged(http:Request request) returns boolean | error {
    ...
    json[] engagedPathsJson = <json[]> sr.readJson();
    ...
    int i = 0;
    while (i < pathLength) {
        map<json> engagedPath = <map<json>> engagedPathsJson[i];
        string configuredPath = check string.constructFrom(engagedPath["path"]);
        string configuredMethod = check string.constructFrom(engagedPath["method"]);
        log:printDebug("Evaluating configured path entry : " + configuredPath + ",  with method : " + configuredMethod);
        boolean isPathMatch = stringutils:equalsIgnoreCase(rawPath, configuredPath);
        boolean isMethodMatch = stringutils:equalsIgnoreCase(method, configuredMethod);
