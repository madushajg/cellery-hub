...
public function getTokenFromDockerAuth(string userName, string token, string registryScope) returns @untainted string | error? {
    log:printDebug(io:sprintf("Invoking docker auth API for get token for scope : %s", registryScope));

    string | error registryServiceName = check encoding:encodeUriComponent(DOCKER_REGISTRY_SERVICE_NAME, "utf-8");
    ...
    if (registryServiceName is string) {
        ...
        if (authResponse is http:Response) {
            map<json> authPayload = <map<json>> authResponse.getJsonPayload();            
