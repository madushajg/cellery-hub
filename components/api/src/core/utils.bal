...
public function deleteImageFromResitry(string orgName, string imageName) returns error? {
    if (orgName != "" && imageName != "") {
        string imageDirectoryPath = io:sprintf("%s/%s/%s", DOCKER_REGISTRY_REPOSITORIES_FILEPATH, orgName,
        imageName);
        ...
        if (file:exists(imageDirectoryPath)) {
            error? removeResults = file:remove(imageDirectoryPath, true);
...
public function deleteOrganizationFromResitry(string orgName) returns error? {
    if (orgName != "") {
        string orgDirectoryPath = io:sprintf("%s/%s", DOCKER_REGISTRY_REPOSITORIES_FILEPATH, orgName);
        log:printDebug(io:sprintf("Deleting the organization \'%s\' from the registry. Organization directory : %s",
        orgName, orgDirectoryPath));

        if (file:exists(orgDirectoryPath)) {
            error? removeResults = file:remove(orgDirectoryPath, true);
            if (removeResults is error) {
